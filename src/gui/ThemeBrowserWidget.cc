...
ThemeBrowserWidget::ThemeBrowserWidget(QWidget* parent) : QWidget(parent) {
...
void ThemeBrowserWidget::ConnectSignals() {
  connect(refresh_button_, &QPushButton::clicked, this,
          &ThemeBrowserWidget::OnRefreshClicked);
  connect(search_edit_, &QLineEdit::textChanged, this,
          &ThemeBrowserWidget::OnSearchChanged);
  connect(file_list_, &QListWidget::itemSelectionChanged, this,
          &ThemeBrowserWidget::OnThemeSelected);
  ...
  connect(apply_button_, &QPushButton::clicked, this,
          &ThemeBrowserWidget::OnApplyClicked);
  connect(delete_button_, &QPushButton::clicked, this,
          &ThemeBrowserWidget::OnDeleteClicked);
}
...
void ThemeBrowserWidget::ScanForThemeFiles() {
  ...
  status_label_->setText(
      QString("Found %1 theme files").arg(theme_files_.size()));
}
            ...
            if (key == "SECONDARY_COLOUR") {
              ...
              if (key == "FONT_default" || key == "FONT_global") {
                const QStringList parts =
                    QString::fromStdString(value).trimmed().split(',');
                if (!parts.isEmpty()) {
...
void ThemeBrowserWidget::OnRefreshClicked() { RefreshThemeFiles(); }
...
void ThemeBrowserWidget::UpdateThemePreview(const ThemeFileInfo& info) {
  ...
  preview += "<p><b>Body:</b> " + info.default_font_family + " (" +
             QString::number(info.default_font_size) + "pt)</p>";
  preview += "<p><b>Heading:</b> " + info.heading_font_family + " (" +
             QString::number(info.heading_font_size) + "pt)</p>";
  preview += "<p><b>Code:</b> " + info.code_font_family + " (" +
             QString::number(info.code_font_size) + "pt)</p>";

  preview +=
      "<div style='padding: 10px; border: 1px solid #e8dcc8; border-radius: "
      "6px; background-color: #fff;'>";
  preview +=
      "<div style='font-family:" + CSSFontFamily(info.heading_font_family) +
      "; font-size:" + QString::number(info.heading_font_size) +
      "pt; font-weight:700;'>"
      "Heading Preview: The Bread Bin</div>";
  preview += "<div style='margin-top:8px; font-family:" +
             CSSFontFamily(info.default_font_family) +
             "; font-size:" + QString::number(info.default_font_size) +
             "pt;'>"
             "Body Preview: Pack my box with five dozen liquor jugs.</div>";
  preview += "<div style='margin-top:8px; font-family:" +
             CSSFontFamily(info.code_font_family) +
             "; font-size:" + QString::number(info.code_font_size) +
             "pt;'>"
