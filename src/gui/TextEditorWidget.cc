...
namespace BreadBin {
    ...
    namespace {
        ...
        public:
            explicit ScriptTerminalDialog (QWidget* parent = nullptr)
                ...
                output_->setStyleSheet("background-color: #121212; color: #e6e6e6; font-family: monospace;");
                ...
                connect(send_button, &QPushButton::clicked, this, &ScriptTerminalDialog::SendInput);
                connect(input_, &QLineEdit::returnPressed, this, &ScriptTerminalDialog::SendInput);
                ...
                connect(process_, &QProcess::finished, this, [this](int exit_code, QProcess::ExitStatus status) {
                    const QString status_text = status == QProcess::NormalExit ? "Normal" : "Crash";
                    AppendOutput(QString("\n[process exited: code=%1, status=%2]\n").arg(exit_code).arg(status_text));
            ...
            void highlightBlock(const QString& text) override {
                switch (mode_) {
                    case Mode::Script:
                        ...
                        break;
                    case Mode::Theme:
                        HighlightKeyValue(text, QRegularExpression("^(NAME|PRIMARY_COLOUR|SECONDARY_COLOUR|BACKGROUND_COLOUR|TEXT_COLOUR|TEXT_COLOR|ACCENT_COLOUR|COLOUR_[A-Za-z_]+|FONT_[A-Za-z_]+|USE_ELEMENT_FONTS)\\s*[:=]"));
                        break;
                    case Mode::Loaf:
                        HighlightKeyValue(text, QRegularExpression("^(TYPE|ID|NAME|PATH|META_COUNT|META_KEY|META_VALUE)\\s*:"));
                        break;
                    case Mode::Json:
                        HighlightJson(text);
                        break;
        ...
        private:
            void HighlightScript(const QString& text) {
                static const QStringList keywords = {
                    ...
                    "def", "class", "import", "from", "try", "except", "finally", "with", "async", "await"
                };
                for (const auto& keyword : keywords) {
                    QRegularExpression keyword_regex(QString("\\b%1\\b").arg(QRegularExpression::escape(keyword)));
                ...
                if (comment_match.hasMatch()) {
                    setFormat(comment_match.capturedStart(), comment_match.capturedLength(), comment_format_);
                }
            ...
            void HighlightKeyValue(const QString& text, const QRegularExpression& key_regex) {
                const auto key_match = key_regex.match(text);
                if (key_match.hasMatch()) {
                    setFormat(key_match.capturedStart(), key_match.capturedLength(), keyword_format_);
                    const int separator_pos = text.indexOf(QRegularExpression("[:=]"));
                    if (separator_pos >= 0 && separator_pos + 1 < text.size()) {
                        setFormat(separator_pos + 1, text.size() - separator_pos - 1, value_format_);
                    }
                }
            ...
            void HighlightIni(const QString& text) {
                ...
                if (section_match.hasMatch()) {
                    setFormat(section_match.capturedStart(), section_match.capturedLength(), keyword_format_);
                    return;
                }
        ...
        GenericSyntaxHighlighter::Mode ToHighlightMode(TextEditorWidget::DocumentType type) {
            switch (type) {
    ...
    void TextEditorWidget::showEvent (QShowEvent* event) {
        ...
        status_label_->setText("Created new " + SuggestedExtensionForType(type) + " file");
    }
    ...
    void TextEditorWidget::ConnectSignals ( ) {
        ...
        connect(run_script_button_, &QPushButton::clicked, this, &TextEditorWidget::OnRunScript);
    ...
    void TextEditorWidget::OnOpenFile ( ) {
        const QString filepath = QFileDialog::getOpenFileName(this, "Open File", "", "All Files (*)");
        if (!filepath.isEmpty()) {
            OpenFile(filepath);
        }
    }
    ...
    bool TextEditorWidget::OpenFile (const QString &filepath) {
        ...
        if (editor && editor->toPlainText().isEmpty() && file_paths_[current_index].isEmpty()) {
            ...
            if (current_index >= 0 && current_index < static_cast<int>(document_types_.size())) {
    ...
    void TextEditorWidget::OnSaveFile() {
        ...
        if (filepath.isEmpty()) {
            ...
            if (detected_type == DocumentType::Script && index < script_loaf_names_.size() && !script_loaf_names_[index].isEmpty()) {
            ...
            const QString default_name = tab_name.isEmpty() ? QString("untitled") : tab_name;
            QString extension = index < preferred_extensions_.size() ? preferred_extensions_[index] : SuggestedExtensionForType(detected_type);
    ...
    bool TextEditorWidget::SaveFile (const QString &filepath) {
        ...
        if (current_index >= 0) {
            ...
            document_types_[current_index] = DetectDocumentType(filepath, editor->toPlainText());
            preferred_extensions_[current_index] = "." + QFileInfo(filepath).suffix();
    ...
    void TextEditorWidget::OnRunScript ( ) {
        ...
        if (document_types_[current_index] != DocumentType::Script) {
            QMessageBox::information(this, "Test Run", "Current file is not detected as a script.");
            return;
        }

        if (file_paths_[current_index].isEmpty()) {
            QMessageBox::information(this, "Test Run", "Please save the script before running it.");
            return;
        }
        ...
        terminal.exec();
        status_label_->setText("Test run finished for: " + QFileInfo(script_path).fileName());
    }
    ...
    void TextEditorWidget::OnFind ( ) {
        ...
        bool ok = false;
        const QString search_text = QInputDialog::getText(this, "Find", "Search for:", QLineEdit::Normal, "", &ok);
    ...
    void TextEditorWidget::OnReplace ( ) {
        ...
        bool ok = false;
        const QString search_text = QInputDialog::getText(this, "Replace", "Search for:", QLineEdit::Normal, "", &ok);
        if (!ok || search_text.isEmpty()) {
            return;
        }

        const QString replace_text = QInputDialog::getText(this, "Replace", "Replace with:", QLineEdit::Normal, "", &ok);
        if (!ok) {
            return;
        }
    ...
    TextEditorWidget::DocumentType TextEditorWidget::DetectDocumentType (const QString& filepath, const QString& content) const {
        const QString suffix = QFileInfo(filepath).suffix().toLower();

        if (suffix == "theme") return DocumentType::Theme;
        if (suffix == "loaf") return DocumentType::Loaf;
        if (suffix == "sh" || suffix == "py" || suffix == "rb" || suffix == "pl" || suffix == "ps1" || suffix == "bat" || suffix == "cmd") return DocumentType::Script;
        ...
        if (suffix == "ini" || suffix == "cfg" || suffix == "conf") return DocumentType::Ini;

        if (content.contains("NAME=") && content.contains("PRIMARY_COLOUR=")) {
            return DocumentType::Theme;
        }

        if (content.contains("TYPE:") && content.contains("ID:") && content.contains("META_COUNT:")) {
            return DocumentType::Loaf;
        }

        if (content.startsWith("#!/") || content.contains("import ") || content.contains("def ") || content.contains("echo ")) {
    ...
    QString TextEditorWidget::SuggestedDirectoryForType (DocumentType type) const {
        const QString docs = QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + "/BreadBin";
    ...
    TextEditorWidget::DocumentType TextEditorWidget::PromptForDocumentType (bool* accepted, bool scripts_only) const {
        QStringList options;
        if (scripts_only) {
            options << "Script";
        }
        else {
            options << "Plain text (.txt)" << "Script" << "Theme (.theme)" << "Loaf (.loaf)"
                    << "JSON (.json)" << "YAML (.yaml)" << "XML (.xml)" << "INI (.ini)";
        }

        bool ok = false;
        const QString choice = QInputDialog::getItem(
            const_cast<TextEditorWidget*>(this),
            "New File Type",
    ...
    QString TextEditorWidget::PromptForScriptExtension (bool* accepted) const {
        QStringList options = {"Shell (.sh)", "Python (.py)", "Ruby (.rb)", "Perl (.pl)", "PowerShell (.ps1)", "Batch (.bat)", "Command (.cmd)"};
        bool ok = false;
        const QString choice = QInputDialog::getItem(
            const_cast<TextEditorWidget*>(this),
            "Script Type",
    ...
    TextEditorWidget::FirstOpenAction TextEditorWidget::PromptForFirstOpenAction ( ) const {
        ...
        QPushButton* open_button = actions->addButton("Open Existing File", QDialogButtonBox::ActionRole);
        QPushButton* new_button = actions->addButton("Create New File", QDialogButtonBox::ActionRole);
