...
bool ParseThemeEntry(const std::string& line, std::string* key,
                     std::string* value) {
...
QString ParseThemeColourString(const std::string& raw_value,
...
MainWindow::MainWindow(QWidget* parent)
  ...
  if (settings.is_open()) {
    ...
    if (!saved_theme_path.empty() &&
        QFile::exists(QString::fromStdString(saved_theme_path))) {
      ApplyThemeFromFile(QString::fromStdString(saved_theme_path), false);
    }
  }

  statusBar()->showMessage(
      "Ready - default loaf folder: " + GetDefaultLoafDirectory(), 8000);
}
...
void MainWindow::CreateCentralWidget() {
  ...
  connect(home_widget_, &HomeWidget::OpenLoafRequested, this,
          &MainWindow::OnOpenLoafFromBrowser);
  connect(home_widget_, &HomeWidget::OpenLoafInTextEditorRequested, this,
          &MainWindow::OnOpenLoafInTextEditor);
  connect(home_widget_, &HomeWidget::NewLoafRequested, this,
          &MainWindow::NewLoaf);

  connect(app_browser_, &AppBrowserWidget::AddApplicationRequested, this,
          &MainWindow::OnAddApplicationFromBrowser);
  connect(loaf_browser_, &LoafBrowserWidget::LoafOpened, this,
          &MainWindow::OnOpenLoafFromBrowser);
  connect(theme_browser_, &ThemeBrowserWidget::ThemeApplied, this,
          &MainWindow::OnApplyThemeFromBrowser);
  connect(theme_editor_, &ThemeEditorWidget::ThemeApplied, this,
          &MainWindow::OnApplyThemeFromBrowser);
  connect(loaf_editor_, &LoafEditorWidget::LoafModified, this, [this]() {
    runtime_widget_->SetLoaf(loaf_editor_->GetCurrentLoaf());
  });
  connect(loaf_editor_, &LoafEditorWidget::CreateNewScriptRequested, this,
          [this](const QString& loaf_name) {
            tab_widget_->setCurrentWidget(text_editor_);
            text_editor_->NewScriptFile(loaf_name);
            statusBar()->showMessage(
                "Create your script in the text editor, then save it", 5000);
          });
...
void MainWindow::CreateToolBar() {
  ...
  connect(save_tool_action, &QAction::triggered, this,
          &MainWindow::SaveCurrentContext);
...
void MainWindow::OpenLoaf() {
  ...
  QString filepath =
      QFileDialog::getOpenFileName(this, "Open Loaf", GetDefaultLoafDirectory(),
...
void MainWindow::SaveLoafAs() {
  ...
  QString filepath = QFileDialog::getSaveFileName(
      this, "Save Loaf As", default_path, "Loaf Files (*.loaf);;All Files (*)");
...
void MainWindow::ShowAbout() {
  QMessageBox::about(this, "About Bread Bin",
                     "<h2>Bread Bin</h2>"
                     "<p><b>Version 0.1</b></p>"
                     "<p>A cross-platform application for organizing, "
                     "managing, and controlling loafs — "
                     "self-contained libraries of applications, files, "
                     "scripts, and configuration.</p>"
                     "<p>Designed for Linux, Windows, and Android with a warm, "
                     "minimal interface.</p>");
}
...
void MainWindow::OnAddApplicationFromBrowser(const AppInfo& app_info) {
  ...
  statusBar()->showMessage(
      "Added application: " + QString::fromStdString(app_info.name), 3000);
}
...
void MainWindow::ApplyThemeFromFile(const QString& filepath,
  ...
  while (std::getline(file, line)) {
    ...
    if (key == "TEXT_COLOUR" || key == "TEXT_COLOR" ||
        key == "COLOUR_foreground" || key == "COLOUR_default") {
    ...
    if (key == "COLOUR_code") {
      editor_background_colour =
          ParseThemeColourString(value, editor_background_colour);
      continue;
    }
    ...
    if (key == "FONT_global" || key == "FONT_default") {
      ...
      if (comma_index != std::string::npos) {
        const QString parsed_family =
            QString::fromStdString(value.substr(0, comma_index)).trimmed();
        if (!parsed_family.isEmpty()) {
          default_font_family = parsed_family;
        }

        const QString raw_size =
            QString::fromStdString(value.substr(comma_index + 1)).trimmed();
  ...
  QString style_sheet =
      QString(R"(
          ...
          .arg(background_colour, primary_colour, secondary_colour, text_colour,
               accent_colour, editor_background_colour);
