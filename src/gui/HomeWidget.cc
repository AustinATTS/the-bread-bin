...
namespace BreadBin {
    ...
    void HomeWidget::SetupUI ( ) {
        ...
        QLabel *welcome_label = new QLabel("<h1 style='color: #5d4e37; font-weight: 600;'>🍞 Welcome to Bread Bin</h1>", this);
        welcome_label->setAlignment(Qt::AlignCenter);
        welcome_label->setStyleSheet("padding: 20px; background-color: transparent;");
        main_layout->addWidget(welcome_label);

        QLabel *subtitle_label = new QLabel("Organize and manage your applications, files, and workflows", this);
        subtitle_label->setAlignment(Qt::AlignCenter);
        subtitle_label->setStyleSheet("color: #8b7a5e; font-size: 14px; padding-bottom: 10px;");
        ...
        connect(new_loaf_btn, &QPushButton::clicked, this, &HomeWidget::NewLoafRequested);
        ...
        content_layout->addWidget(recent_group, 1);  // Stretch factor for responsive sizing
        ...
        content_layout->addWidget(favorites_group, 1);  // Stretch factor for responsive sizing
        ...
        content_layout->addWidget(categories_group, 1);  // Stretch factor for responsive sizing
    ...
    void HomeWidget::ConnectSignals ( ) {
        connect(recent_list_, &QListWidget::itemDoubleClicked,
                this, &HomeWidget::OnRecentLoafDoubleClicked);
        connect(favorites_list_, &QListWidget::itemDoubleClicked,
                this, &HomeWidget::OnFavoriteLoafDoubleClicked);
        connect(categories_list_, &QListWidget::itemDoubleClicked,
                this, &HomeWidget::OnCategoryLoafDoubleClicked);

        connect(recent_list_, &QListWidget::customContextMenuRequested,
                this, &HomeWidget::ShowContextMenu);
        connect(favorites_list_, &QListWidget::customContextMenuRequested,
                this, &HomeWidget::ShowContextMenu);
        connect(categories_list_, &QListWidget::customContextMenuRequested,
                this, &HomeWidget::ShowContextMenu);
    }
    ...
    void HomeWidget::LoadSettings ( ) {
        ...
        if (root.contains("recent") && root["recent"].isArray()) {
            ...
            for (const QJsonValue& value : recent) {
                ...
                info.last_opened = QDateTime::fromString(obj["lastOpened"].toString(), Qt::ISODate);
    ...
    void HomeWidget::UpdateRecentList ( ) {
        ...
        if (recent_loafs_.isEmpty()) {
            QListWidgetItem *item = new QListWidgetItem("No recent loafs", recent_list_);
    ...
    void HomeWidget::UpdateFavoritesList ( ) {
        ...
        if (favorites_list_->count() == 0) {
            QListWidgetItem *item = new QListWidgetItem("No favorites yet", favorites_list_);
            item->setFlags(Qt::NoItemFlags);
        }
    }
    ...
    void HomeWidget::UpdateCategoriesList ( ) {
        ...
        for (auto it = categories.begin(); it != categories.end(); ++it) {
            QListWidgetItem *header = new QListWidgetItem("📁 " + it.key(), categories_list_);
        ...
        if (categories_list_->count() == 0) {
            QListWidgetItem *item = new QListWidgetItem("No categorized loafs", categories_list_);
            item->setFlags(Qt::NoItemFlags);
        }
    }
    ...
    void HomeWidget::ShowContextMenu (const QPoint& pos) {
        ...
        bool is_favorite = all_loafs_.contains(filepath) && all_loafs_[filepath].is_favorite;
        QAction *favorite_action = menu.addAction(is_favorite ? "Remove from Favorites" : "Add to Favorites");
            ...
            if (selected == open_text_action) {
                ...
                if (selected == favorite_action) {
                    ...
                    if (selected == category_action) {
                        bool ok;
                        QString current_category = all_loafs_.contains(filepath) ? all_loafs_[filepath].category : "";
                        QString category = QInputDialog::getText(
                            this, "Set Category", "Category name:",
                            QLineEdit::Normal, current_category, &ok
                        );
                        if (ok) {
