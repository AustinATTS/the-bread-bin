...
LoafEditorWidget::LoafEditorWidget(QWidget* parent)
    : QWidget(parent), editor_(std::make_shared<LoafEditor>()) {
  SetupUI();
  ConnectSignals();

  // Initialize with a default empty loaf so users can start adding items
  // immediately
...
void LoafEditorWidget::SetupUI() {
  ...
  name_edit_->setToolTip(
      "The name of this loaf (e.g., 'Development Environment', 'Media Suite')");
  ...
  description_edit_->setPlaceholderText(
      "Enter a description of what this loaf contains");
  description_edit_->setToolTip(
      "A brief description of the purpose and contents of this loaf");
  ...
  item_list_->setToolTip(
      "List of all items in this loaf. Select an item and click 'Configure' to "
      "set up dependencies and custom settings.");
  ...
  add_app_button_->setToolTip(
      "Browse for an application executable to add to this loaf");
  ...
  add_script_button_->setToolTip(
      "Browse for a script file (*.sh, *.bat, *.py, etc.)");
  add_script_button_->setMinimumWidth(80);

  add_config_button_ = new QPushButton("⚙️ Config", this);
  add_config_button_->setToolTip(
      "Browse for a configuration file (*.conf, *.ini, *.json, etc.)");
  ...
  configure_button_->setToolTip(
      "Configure dependencies and custom settings for the selected item");
...
void LoafEditorWidget::ConnectSignals() {
  connect(name_edit_, &QLineEdit::textChanged, this,
          &LoafEditorWidget::OnNameChanged);
  connect(description_edit_, &QTextEdit::textChanged, this,
          &LoafEditorWidget::OnDescriptionChanged);
  connect(add_app_button_, &QPushButton::clicked, this,
          &LoafEditorWidget::OnAddApplication);
  connect(add_file_button_, &QPushButton::clicked, this,
          &LoafEditorWidget::OnAddFile);
  connect(add_script_button_, &QPushButton::clicked, this,
          &LoafEditorWidget::OnAddScript);
  connect(add_config_button_, &QPushButton::clicked, this,
          &LoafEditorWidget::OnAddConfig);
  connect(add_web_page_button_, &QPushButton::clicked, this,
          &LoafEditorWidget::OnAddWebPage);
  connect(remove_button_, &QPushButton::clicked, this,
          &LoafEditorWidget::OnRemoveItem);
  connect(configure_button_, &QPushButton::clicked, this,
          &LoafEditorWidget::OnConfigureItem);
  connect(item_list_, &QListWidget::itemSelectionChanged, this,
          &LoafEditorWidget::OnItemSelected);
}
...
void LoafEditorWidget::NewLoaf() {
  ...
  QString name = QInputDialog::getText(
      this, "New Loaf", "Enter loaf name:", QLineEdit::Normal, "My Loaf", &ok);
...
void LoafEditorWidget::OnAddApplication() {
  ...
  QLabel* label =
      new QLabel("Select an application or enter path manually:", &dialog);
  layout->addWidget(label);

  QGroupBox* select_group =
      new QGroupBox("Select Installed Application", &dialog);
  ...
  connect(app_combo, qOverload<int>(&QComboBox::currentIndexChanged), [&](int) {
    ...
    name_edit->setText(info.completeBaseName().isEmpty()
                           ? app_combo->currentText()
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (path.isEmpty()) {
      QMessageBox::warning(this, "Error",
                           "Please select or enter an application path");
      return;
    }
    ...
    if (editor_->AddApplication(id.toStdString(), name.toStdString(),
                                path.toStdString())) {
      RefreshItemList();
...
void LoafEditorWidget::OnAddFile() {
  QString path = QFileDialog::getOpenFileName(
      this, "Select File", QDir::homePath(), "All Files (*)");
  ...
  name = QInputDialog::getText(this, "Add File",
                               "Display Name:", QLineEdit::Normal, name, &ok);
  if (!ok || name.isEmpty()) {
    name = info.fileName();
  }
  ...
  if (editor_->AddFile(id.toStdString(), name.toStdString(),
                       path.toStdString())) {
    RefreshItemList();
...
void LoafEditorWidget::OnAddScript() {
  ...
  QPushButton* browse_btn =
      new QPushButton("Browse for Existing Script", &dialog);
  ...
  bool ok;
  name = QInputDialog::getText(this, "Add Script",
                               "Display Name:", QLineEdit::Normal, name, &ok);
  if (!ok || name.isEmpty()) {
    name = info.fileName();
  }
  ...
  if (editor_->AddScript(id.toStdString(), name.toStdString(),
                         path.toStdString())) {
    RefreshItemList();
...
void LoafEditorWidget::OnAddConfig() {
  QString path =
      QFileDialog::getOpenFileName(this, "Select Config File", QDir::homePath(),
                                   "Config Files (*.conf *.cfg *.ini *.json "
                                   "*.xml *.yaml *.yml *.toml);;All Files (*)");
  ...
  bool ok;
  name = QInputDialog::getText(this, "Add Config",
                               "Display Name:", QLineEdit::Normal, name, &ok);
  if (!ok || name.isEmpty()) {
    name = info.fileName();
  }
  ...
  if (editor_->AddConfig(id.toStdString(), name.toStdString(),
                         path.toStdString())) {
    RefreshItemList();
...
void LoafEditorWidget::OnAddWebPage() {
  bool ok;
  QString url = QInputDialog::getText(this, "Add Web Page",
  ...
  QString name = QInputDialog::getText(
      this, "Add Web Page", "Display Name:", QLineEdit::Normal, url, &ok);
  ...
  if (editor_->AddWebPage(id.toStdString(), name.toStdString(),
                          url.toStdString())) {
    RefreshItemList();
...
void LoafEditorWidget::OnRemoveItem() {
  ...
  if (!depends_on.empty() || !args.empty() || !working_dir.empty() ||
      auto_start == "true") {
    has_configs = true;
  }

  QString message = "Are you sure you want to remove '" +
                    QString::fromStdString(loaf_item->GetName()) + "'?";
  if (has_configs) {
    message += "\n\nThis item has custom configurations that will be lost.";
  }

  QMessageBox::StandardButton reply = QMessageBox::question(
      this, "Confirm Remove", message, QMessageBox::Yes | QMessageBox::No);
...
void LoafEditorWidget::OnConfigureItem() {
  ...
  QDialog dialog(this);
  dialog.setWindowTitle("Configure Item: " +
                        QString::fromStdString(loaf_item->GetName()));
  ...
  wd_edit->setText(
      QString::fromStdString(loaf_item->GetMetadata("working_dir")));
  ...
  connect(wd_browse, &QPushButton::clicked, [&]() {
    QString dir =
        QFileDialog::getExistingDirectory(&dialog, "Select Working Directory");
    if (!dir.isEmpty()) {
      wd_edit->setText(dir);
    }
  });
  ...
  QCheckBox* auto_start_check =
      new QCheckBox("Auto-start when loaf runs", &dialog);
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    loaf_item->SetMetadata("auto_start",
                           auto_start_check->isChecked() ? "true" : "false");

    status_label_->setText("Updated configuration for: " +
                           QString::fromStdString(loaf_item->GetName()));
    emit LoafModified();
  }
}
