...
AppBrowserWidget::AppBrowserWidget(QWidget* parent)
    : QWidget(parent), discovery_(std::make_shared<AppDiscovery>()) {
  SetupUI();
  ConnectSignals();
...
void AppBrowserWidget::SetupUI() {
  ...
  search_label->setStyleSheet(
      "font-weight: 600; font-size: 13px; min-width: 70px;");
  ...
  status_label_ =
      new QLabel("Ready. Click 'Scan System' to discover applications.", this);
...
void AppBrowserWidget::ConnectSignals() {
  connect(scan_button_, &QPushButton::clicked, this,
          &AppBrowserWidget::OnScanClicked);
  connect(search_edit_, &QLineEdit::textChanged, this,
          &AppBrowserWidget::OnSearchChanged);
  connect(category_combo_,
          QOverload<const QString&>::of(&QComboBox::currentTextChanged), this,
          &AppBrowserWidget::OnCategoryChanged);
  connect(app_list_, &QListWidget::itemSelectionChanged, this,
          &AppBrowserWidget::OnApplicationSelected);
  connect(add_button_, &QPushButton::clicked, this,
          &AppBrowserWidget::OnAddToLoafClicked);
}

void AppBrowserWidget::OnScanClicked() {
  QProgressDialog progress("Scanning system for applications...", "Cancel", 0,
                           0, this);
...
void AppBrowserWidget::OnAddToLoafClicked() {
  ...
  if (app) {
    ...
    status_label_->setText("Application added: " +
                           QString::fromStdString(app->name));
  }
}

void AppBrowserWidget::RefreshApplications() { UpdateApplicationList(); }
...
void AppBrowserWidget::UpdateApplicationDetails(const AppInfo& app_info) {
  ...
  if (!app_info.description.empty()) {
    details += "<p><b>Description:</b> " +
               QString::fromStdString(app_info.description) + "</p>";
  }

  details += "<p><b>Executable:</b> <code>" +
             QString::fromStdString(app_info.executable) + "</code></p>";

  if (!app_info.category.empty()) {
    details += "<p><b>Category:</b> " +
               QString::fromStdString(app_info.category) + "</p>";
  }

  if (!app_info.icon_path.empty()) {
    details += "<p><b>Icon:</b> " + QString::fromStdString(app_info.icon_path) +
               "</p>";
  }
  ...
  details +=
      "<li><code>--config &lt;file&gt;</code> - Specify config file</li>";
  details += "</ul>";
...
void AppBrowserWidget::PopulateCategories() const {
  ...
  if (const int index = category_combo_->findText(current_category);
      index >= 0) {
