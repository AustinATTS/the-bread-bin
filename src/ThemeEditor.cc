...
ThemeEditor::Colour ParseColourValue(const std::string& value,
                                     const ThemeEditor::Colour& fallback) {
...
void ResetToDefaults(std::map<std::string, ThemeEditor::Colour>* colours,
...
ThemeEditor::ThemeEditor()
    : theme_name_("Default Theme"), unsaved_changes_(false) {
  ...
  if (ParseThemeEntry(first_line, key, value) && key == "NAME") {
    ...
    while (std::getline(file, line)) {
      ...
      if (key == "BACKGROUND_COLOUR") {
        colours_["background"] =
            ParseColourValue(value, colours_["background"]);
        continue;
      }

      if (key == "TEXT_COLOUR" || key == "TEXT_COLOR") {
        colours_["foreground"] =
            ParseColourValue(value, colours_["foreground"]);
        continue;
      }
...
bool ThemeEditor::SaveTheme(const std::string& filepath) {
  ...
  file << "PRIMARY_COLOUR=" << primary.red << " " << primary.green << " "
       << primary.blue << " " << primary.alpha << "\n";
  file << "SECONDARY_COLOUR=" << secondary.red << " " << secondary.green << " "
       << secondary.blue << " " << secondary.alpha << "\n";
  file << "BACKGROUND_COLOUR=" << background.red << " " << background.green
       << " " << background.blue << " " << background.alpha << "\n";
  file << "TEXT_COLOUR=" << foreground.red << " " << foreground.green << " "
       << foreground.blue << " " << foreground.alpha << "\n";
  file << "ACCENT_COLOUR=" << accent.red << " " << accent.green << " "
       << accent.blue << " " << accent.alpha << "\n";

  for (const auto& key : ColourElementKeys()) {
    const Colour colour = GetColour(key);
    file << "COLOUR_" << key << "=" << colour.red << " " << colour.green << " "
         << colour.blue << " " << colour.alpha << "\n";
  }

  for (const auto& font : fonts_) {
    file << "FONT_" << font.first << "=" << font.second.first << ","
         << font.second.second << "\n";
  }
...
void ThemeEditor::SetFont(const std::string& element,
...
void ThemeEditor::SetStyle(const std::string& property,
...
std::string ThemeEditor::GetThemeName() const { return theme_name_; }

bool ThemeEditor::HasUnsavedChanges() const { return unsaved_changes_; }
