    ...
    try {
        ...
        if (auto s = config["style"].as_table()) {
            auto getFloat = [&](const std::string& key, float def) -> float {
                if (s->contains(key)) return float((*s)[key].value<double>().value_or(def));
                return def;
            };
            auto getVec2 = [&](const std::string& key, const ImVec2& def) -> ImVec2 {
                if (!s->contains(key)) return def;
                auto arr = (*s)[key].as_array();
                if (!arr || arr->size() < 2) return def;
                return ImVec2(
                    float((*arr)[0].value<double>().value_or(def.x)),
                    float((*arr)[1].value<double>().value_or(def.y))
                );
            };
        ...
        if (auto c = config["colors"].as_table()) {
            ...
            for (auto& [key, val] : *c) {
                ...
                if (!val.is_array() || val.as_array()->size() < 4) continue;

                ImGuiCol idx = ImGuiCol_Text;
                if (nameMap.find(name) != nameMap.end()) idx = nameMap[name];

                auto arr = val.as_array();
                colors[idx] = ImVec4(
                    float((*arr)[0].value<double>().value_or(colors[idx].x)),
                    float((*arr)[1].value<double>().value_or(colors[idx].y)),
                    float((*arr)[2].value<double>().value_or(colors[idx].z)),
                    float((*arr)[3].value<double>().value_or(colors[idx].w))
                );
            }
        }
